...
class ItemDefinition:
    ...
    name_jp: str               # name_JP
    name_en: str               # name
    category: str              # category（drink, food など）
...
CUSTOM_ITEMS: Dict[str, ItemDefinition] = {
    # ========================================
    # 業火の接吻（Kiss of Inferno）- 複合耐性
    # ========================================
    "sukutsu_kiss_of_inferno": ItemDefinition(
        id="sukutsu_kiss_of_inferno",
        name_jp="業火の接吻",
        name_en="Kiss of Inferno",
        category="drink",
        detail_jp="背徳の味がする真紅のポーション。複数の耐性を長時間得られるが、魂は汚れる。",
        detail_en="A crimson potion with a sinful taste. Grants multiple resistances, but stains your soul.",
...
def validate_items() -> List[str]:
    ...
    for item_id, item in CUSTOM_ITEMS.items():
        ...
        # 販売設定の整合性
        if item.sell_at and item.stock_rarity not in ["Normal", "Rare", "Legendary", "Mythical"]:
