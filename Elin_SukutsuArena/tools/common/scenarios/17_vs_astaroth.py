...
def define_vs_astaroth(builder: DramaBuilder):
    ...
    builder.step(main) \
        ...
        .shake() \
        .say("narr_3", "（王座の間に続く大扉が粉々に砕け散り、そこから、背中に巨大な紅蓮の翼を湛えた男ーーアスタロトが、重力を無視してゆっくりと降り立つ。）", "", actor=pc) \
    ...
    builder.step(scene2) \
        ...
        .say("narr_5", "（アスタロトは、震えるリリィと、彼女を庇って前に出るバルガスを一瞥し、嘲笑するように鼻を鳴らした。）", "", actor=pc) \
        .focus_chara(Actors.ASTAROTH) \
        .say("astaroth_4", "リリィ……その人間は、もはやお前の手には負えぬバケモノだ。", "", actor=astaroth) \
        .say("astaroth_5", "そしてバルガス、敗残兵の分際で、私に逆らうとはな。活かしてやったことを後悔させたいのか？", "", actor=astaroth) \
        .say("astaroth_6", "（アスタロトはあなた視線を向けた）", "", actor=astaroth) \
        .say("astaroth_6f", "おまえはイルヴァの加護を持っている。システムの想定外。だからこそ、価値があった。……だが、制御できぬバグは、消去するしかない。", "", actor=astaroth) \
        .say("narr_6", "（アスタロトが右手を掲げると、その周囲に凝縮された形而上の炎が渦巻く。）", "", actor=pc) \
        .say("astaroth_7", "さらばだ。お前の物語は『なかったこと』になる。", "", actor=astaroth) \
        .jump(scene3)
    ...
    builder.step(scene3) \
        ...
        .say("narr_7", "（アスタロトの放った一撃が、次元そのものを焼き尽くさんばかりの勢いであなたに迫る。）", "", actor=pc) \
        .shake() \
        .say("narr_9", "（リリィが叫び、バルガスが叫ぶ。その直前ーー。）", "", actor=pc) \
        .say("narr_10", "（空間がガラスのようにパリンと音を立てて割れ、あなたの目の前に、ゼクがひょっこりと現れた。）", "", actor=pc) \
        .focus_chara(Actors.ZEK) \
        .say("narr_11", "（その手には、不気味に明滅し、バグったように色彩が反転し続ける立方体が握られている。）", "", actor=pc) \
    ...
    builder.step(scene4) \
        ...
        .focus_chara(Actors.ZEK) \
        .say("zek_3", "クク……私のコレクションは大したものでしょう？", "", actor=zek) \
        .say("zek_4", "さあ、闘士殿！ リリィもバルガスも連れて、私の影に飛び込みなさい！", "", actor=zek) \
        .say("zek_5", "この道具が保つのはあと数秒だ。アスタロトを倒すための『本当の力』、それを手に入れる猶予を……私が作ってあげましょう！", "", actor=zek) \
