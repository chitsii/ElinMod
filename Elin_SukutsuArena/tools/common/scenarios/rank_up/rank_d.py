...
def define_rank_up_D(builder: DramaBuilder):
    ...
    builder.step(react1_audience) \
        .say("lily_r1", "無理です。彼らは高次元の存在。私たちには手が届きません。……諦めて、楽しませてあげてください。", "", actor=lily) \
        .jump(scene2)
    ...
    builder.step(battle_start) \
        ...
        .wait(0.5) \
        .say("obs_1", "（観客の歓声：「さあ、始めろ！」）", "", actor=pc) \
        .say("obs_2", "（観客の笑い声：「今回の賭けは『3ターン以内に圧死』だ」）", "", actor=pc) \
        .say("obs_3", "（観客の嘲笑：「いや、5ターンは持つだろう。……多分」）", "", actor=pc) \
        .wait(1.0) \
        .say("narr_8", "（戦いが始まった瞬間、頭上の虚空が紫色に光り、何かが降ってくる音が響いた……！）", "", actor=pc) \
        .shake() \
        .start_battle(4, is_rank_up=True, master_id="sukutsu_arena_master") \
        .finish()
