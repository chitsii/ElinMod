...
def define_opening_drama(builder: DramaBuilder):
    ...
    ユーザー要望: セリフを結合して読みやすく、英語テキストを省略
    """
    # アクター登録
    # アクター登録
    ...
    # ナレーション (PCの独白)
    builder.say("narr1", "(色彩は混濁し、上下左右の感覚が溶けていく。次に足が触れたのは、石畳だった。)", actor=pc) \
           .say("narr2", "(それは、数多の敗者の絶望が凝固し、永遠に熱を失ったかのような黒曜石だった。頭上を見上げれば、そこには空も太陽もない。)", actor=pc) \
           .say("narr3", "(ただ、紫紺の渦を巻く「虚無」が天を覆い、時折、空間の裂け目から巨大な鎖が垂れ下がっている。遠くから響くのは、雷鳴のような喝采。だが、その声には「熱」がない。\n高次元の観客たちが、ただ死を消費するために発する、冷ややかな振動だ。)", actor=pc)

    # リリィ
    builder.say("lily1", "……あら。珍しいこともあるものですね。召喚の儀も、空間の歪みもなしに、この『ヴォイド・コロシアム』に迷い込む『生きた肉』がいるなんて。", actor=lily) \
           .say("lily2", "お客様、それとも……新たな『商品』かしら？ここは境界の終わり、そして絶望の始まり。", actor=lily) \
           .say("lily3", "あなたがどこから来たのかは問いません。入られた方は皆、あそこにいる「飲んだくれ」に聞くのが、ここの作法ですから。", actor=lily)

    # バルガス
    builder.say("vargus1", "……ケッ、シケた面してやがる。おい、サキュバス。そんなひょろいガキ、鑑定するまでもねえ。", actor=vargus) \
    ...
    builder.step(vargus_react) \
           .say("vargus_react1", "……ハッ、そうかよ！ この地獄の底に立った時点で、貴様はもう『闘士』だ。", actor=vargus) \
